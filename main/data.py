...
def create_table(path):
    connection = sqlite3.connect(path)
    cur = connection.cursor()
    query = '''CREATE TABLE IF NOT EXISTS USERS(username TEXT PRIMARY KEY,password TEXT, status TEXT, port INT)'''
    cur.execute(query)
    connection.commit()
    cur.close()
    connection.close()

def insert_to_db(cur,username, password, status, port):
    try:
        query = '''INSERT INTO users VALUES(?,?,?,?)'''
        cur.execute(query,(username,password, status, port))
...
def store_new_info(username, password, status, port, path):
    ...
    if(check_username(username,path)):
        ...
        if insert_to_db(cur,username, password,status, port):
            cur.close()
            connection.commit()
            connection.close()
            return True
